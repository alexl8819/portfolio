...
export const ProjectCardSkeleton = memo(({ tags = 8 }: { tags?: number }) => {
    return (
        <Card className='min-h-64 flex flex-col xl:flex-row bg-white'>
            <div className='h-64 xl:h-84 motion-safe:animate-pulse bg-zinc-600 xl:w-1/3'></div>
            <div className='flex flex-col w-full xl:w-3/4'>
                <CardHeader className='mt-3 mb-6 px-6 h-28'>
                    <div className='w-40 h-6 motion-safe:animate-pulse bg-zinc-600 my-2'></div>
                    <p className='h-6 xl:mr-28 line-clamp-3 motion-safe:animate-pulse bg-zinc-600'></p>
                </CardHeader>
                    ...
                    <div className='flex flex-col justify-between h-full'>
                            <div className="flex flex-wrap gap-2">
                            {
						        Array.from({ length: tags }).map((_, i) => (
                        	        <span
                          		        key={i}
                          		        className="h-5 w-10 px-2.5 py-0.5 rounded-full motion-safe:animate-pulse bg-gray-600"
                        	        >
                        	        </span>
                      	        ))
					        }
                            ...
                            <div className="mt-4 flex justify-evenly xl:justify-start items-center xl:space-x-3">
                                {
                                    Array.from({ length: 3 }).map((_, i) => (
                                        <div key={i} className='rounded-lg w-36 h-8 motion-safe:animate-pulse bg-zinc-600'></div>
                                    ))
                                }
                            </div>
                    </div>
...
export const CertificationSkeleton = memo(({}) => {
    return (
        <div 
            className="bg-white dark:bg-stone-900 p-6 rounded-lg select-none border border-zinc-100 dark:border-neutral-700 opacity-80 hover:opacity-100 shadow-sm hover:shadow-md transition-shadow"
        >
            <div className="flex flex-col md:flex-row md:items-center justify-between">
                <div className="flex flex-col md:flex-row items-center space-x-0 md:space-x-4">
                    ...
                    <div className='mt-3 lg:mt-0 space-y-2'>
                        ...
                        <div className="h-5 w-24 rounded-lg motion-safe:animate-pulse bg-zinc-600"></div>
                    </div>
                </div>
                <div className='flex justify-center items-center'>
                    <div className='flex flex-row justify-center items-center mt-2 md:mt-0 rounded-lg h-12 w-40 motion-safe:animate-pulse bg-zinc-600'></div>
                </div>
            </div>
            <div className="mt-4 h-6 w-full rounded-lg motion-safe:animate-pulse bg-zinc-600"></div>
        </div>
    );
});
