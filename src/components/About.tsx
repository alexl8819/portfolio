    ...
    return (
        <div className="px-4 md:px-6">
            <div className="max-w-3xl mx-auto">
                <h2 className="text-3xl font-playfair-display font-bold tracking-tight mb-8 flex justify-center items-center">
              	    About Me
                </h2>
			    {
				    blocks.map((block, index) =>(
					    <p key={index} className={`text-zinc-600 ${ index !== (blocks.length - 1) ? 'mb-6' : ''}`}>
                            { block }
                        </p>
				    ))
			    }
